...
config.rs                      # Config { tab_width, scroll_off, wrap, gui_font_size, gui_font_family } + ConfigFile(Deserialize) + Config::load()
...
main.rs                        # TUIエントリポイント。Config::load() → App::new(path, config_result)
...
ui/mod.rs                      # render()。全UIコンポーネントをレイヤ描画
  ...
  ui/workspace_symbols.rs     # ワークスペースシンボル検索ポップアップ
  ui/tab_bar.rs                # マルチバッファのタブバー

# GUI フロントエンド (#[cfg(feature = "gui")])
gui_main.rs                    # GUIエントリポイント。Config::load() → GuiApp::new(cc, path, config_result)
gui_app.rs (GuiApp)            # eframe::App実装。tokio::runtime::Runtime + mpscブリッジでLSP統合 + マウス操作 + カスタムフォント登録
gui/mod.rs                     # render()→Vec<(pane_id, Rect)>。egui CentralPanel + TopBottomPanel でレイアウト
  ...
  gui/workspace_symbols.rs    # ワークスペースシンボル検索ポップアップ
...
`gui_app.rs`の`handle_mouse()`がマウスイベントを処理:
- **スクロールホイール**: `ctx.input(|i| i.smooth_scroll_delta)` → `Editor::scroll_viewport_up/down()` (ビューポートのみスクロール、カーソルは`clamp_cursor()`で画面内に留める)
- **クリック**: `ctx.input(|i| i.pointer.primary_clicked())` + `interact_pos()`
  - ポップアップ表示中は無視
  - `last_pane_rects`（前フレームの`render()`戻り値）からクリック先ペインを特定
  - 非アクティブペインなら`save_active_pane()` + `load_pane()`でフォーカス切替
  - ステータスライン上→フォーカス切替のみ、ガター上→無視
  - エディタ領域: ピクセル座標→スクリーン座標→ドキュメント座標変換（wrap対応: `build_screen_map()`で逆引き）

**ペイン座標の受け渡し**: `render()`が`Vec<(pane_id, egui::Rect)>`を返し、`GuiApp::last_pane_rects`に保存。次フレームで`handle_mouse()`が参照（即時モードGUI標準手法、1フレーム遅延）。

**ビューポートスクロール**: `Editor::scroll_viewport_down/up(n)` は `View::scroll_down_by/scroll_up_by`（`pub(crate)`）に委譲。`half_page_down/up`と異なりカーソルは動かさない（Vimの`Ctrl-E`/`Ctrl-Y`相当）。
...
| `Ctrl-T` | WorkspaceSymbol | ワークスペースシンボル検索 |
...
| `:set fontsize=N` | GUI版フォントサイズ変更（8〜48、デフォルト14） |
| `:set font=<name>` | GUI版フォントファミリー変更（空でデフォルトに戻す） |
| `:set scrolloff=N` | スクロールオフセット変更（0〜999） |
| `:set tabstop=N` | タブ幅変更（1〜16） |
| `:!<command>` | シェルコマンド実行 |
...
| `serde 1` (derive) | 設定ファイルデシリアライズ + LSP型 | 常時 |
| `serde_json 1` | LSP JSON-RPC + 設定ファイルパース | 常時 |
...
- **ワークスペースシンボル検索** (`Ctrl-T`): `workspace/symbol`（プロジェクト全体のシンボル検索）
...
1. `config.rs`: `Config` にフィールド追加 + `Default` にデフォルト値
2. `config.rs`: `ConfigFile` に同名の `Option<T>` フィールド追加
3. `config.rs`: `load_from_path()` のマージ処理に `unwrap_or` 追加
4. `editor/mod.rs`: `execute_command()` 内に `:set xxx=N` ハンドラ追加（`"set fontsize="` の実装を参考に）
5. 必要に応じて `Editor::with_config()` で初期値を同期（例: `view.wrap = config.wrap`）

## 現在の状態と既知の制限

### ビルド状態
- `cargo build`（TUI版）は成功（警告なし）
- `cargo build --features gui --bin righter-gui`（GUI版）は成功（警告なし）
- `cargo clippy -- -D warnings` 両ビルドとも通過
- `cargo test` は成功（59テスト: editor 34件 + LSP 25件）

### テスト構成
- `tests/editor_tests.rs` — Document操作、カーソル移動、編集操作、undo/redo、モード遷移、Config読み込み
- `tests/lsp_parse_tests.rs` — LSP JSON-RPCレスポンスのパース（diagnostics, completions, hover, goto, references, code actions, workspace symbols, rename edits, URI変換）
- `src/lib.rs` — テスト用ライブラリクレート（共有モジュールをpub re-export）
...
`~/.config/righter/config.json`（`$XDG_CONFIG_HOME` 優先）を起動時に1回読み込み。全フィールドOptional、ファイルなしでも正常起動。`:set` による変更はセッション限り（ファイルへの書き戻しなし）。
...
**初期化フロー**:
```
Config::load()                  # config.rs: XDG_CONFIG_HOME → ~/.config フォールバック
  → ConfigLoadResult { config, warning }
  → Editor::with_config(doc, config)  # editor/mod.rs: view.wrapをconfig.wrapで同期
  → warning → editor.status_message   # app.rs / gui_app.rs
```

**エラーハンドリング**:
| シナリオ | 動作 |
|---------|------|
| ファイルなし | デフォルト値。メッセージなし |
| パーミッションエラー | デフォルト + ステータスバーに警告 |
| JSON構文エラー | デフォルト + パースエラーをステータスバーに表示 |
| 未知のフィールド | serde_json がデフォルトで無視。エラーにならない |

### GUI固有: カスタムフォント

`gui_app.rs`の`find_font_file()` + `setup_custom_font()`でフォントファミリーを変更可能:
- **方式**: `FontFamily::Monospace`に登録されたフォントリストの先頭にユーザー指定フォントを挿入。GUIコンポーネント11ファイルは変更不要
- **フォント検索**: macOSフォントディレクトリを走査（`~/Library/Fonts/`, `/Library/Fonts/`, `/System/Library/Fonts/`, `/System/Library/Fonts/Supplemental/`）。ファイル名ベースの大文字小文字無視マッチ、`.ttf`/`.otf`のみ
- **初期化**: `GuiApp::new()`で`config.gui_font_family`をチェック→`setup_custom_font()`
- **ランタイム変更**: `Editor::font_family_changed`フラグを`update()`でフレームごとにチェック。フォント未発見時はデフォルトにリセット
