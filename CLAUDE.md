...
config.rs                      # Config { tab_width, scroll_off, wrap, gui_font_size } + ConfigFile(Deserialize) + Config::load()
...
gui_app.rs (GuiApp)            # eframe::App実装。tokio::runtime::Runtime + mpscブリッジでLSP統合 + マウス操作
...
- マルチカーソル
- GUI版: フォント設定（ファミリー変更 — サイズは `:set fontsize=N` で対応済み）
