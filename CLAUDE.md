...
### 既知の警告（TUI: 9件, GUI: 12件、全て無害）
- `editor/mod.rs`: `negative` 変数の未読（数値パース処理内）
- `editor/mod.rs`: `new`, `highlight_style_at`, `is_search_match`, `buffer_count`, `scroll_viewport_down/up` が片方のフロントエンドで未使用
- `input/command.rs`: `SelectRegister`, `FormatDocument` バリアントが未使用（将来用）
- `lsp/mod.rs`: 構造体フィールドの未使用警告（`params`, `detail`, `kind`, `end_line`, `end_col`, `command`）
- `lsp/mod.rs`: `LspWorkspaceEdit` 未使用
- GUI固有: `from_crossterm`, `separators`, `shutdown`, `uri_to_path`, `AppEvent::Key/Resize` が未使用（TUI専用コード）
