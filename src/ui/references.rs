...
pub struct ReferencesPopup<'a> {
    editor: &'a Editor,
}

impl<'a> ReferencesPopup<'a> {
    pub fn new(editor: &'a Editor) -> Self {
        Self { editor }
    }
}

impl Widget for ReferencesPopup<'_> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        if !self.editor.showing_references || self.editor.references.is_empty() {
            return;
        }

        let refs = &self.editor.references;
        let selected = self.editor.reference_index;

        // Format reference items: extract filename from URI and show line number
        let items: Vec<String> = refs
            ...
            .map(|r| {
                let name = r
                    ...
                    .next()
                    .unwrap_or(&r.uri);
                format!("{}:{}:{}", name, r.start_line + 1, r.start_col + 1)
            })
            .collect();
        ...
        let normal_style = Style::default().fg(Color::White).bg(Color::Rgb(30, 33, 40));
        ...
        for dy in 0..popup_height {
            ...
            for dx in 0..popup_width {
                ...
                buf[(x, y)].set_char(ch).set_style(if is_border { border_style } else { normal_style });
            }
        }
